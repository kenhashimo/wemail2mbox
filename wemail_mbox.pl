# ========== WeMail → mbox 変換ツール ==========
  ...
  if (@ARGV <= 0) {
    ...
    print STDERR 'Usage: ' . Encode::decode('cp932', $0) . " 区切り文字\n";
    exit 1;
  }
...
sub cur {
  ...
  if ($wem) {
    ...
    $mf =~ s/\\/$kgr/g;  # フォルダの区切り文字を変換

    unless (openL(\$fpr, '>:encoding(cp932)', $mf)) {
      print STDERR "ファイル $mf の書き込みに失敗しました\n";
      return;
    }
    print $fpr $res;
    close $fpr;
  ...
  foreach $fil ($dhw -> readdirL()) {
    ...
    if (substr($fil, -4) eq '.wem') {
      ...
      unless (openL(\$fpi, '<:encoding(cp932)', $fil)) {
        print STDERR "ファイル $fil の読み出しに失敗しました\n";
        next;
      }
